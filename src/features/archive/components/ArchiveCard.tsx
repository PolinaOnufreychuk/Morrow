 ...
 */
export function ArchiveCard({ entry, onRestore, onDeletePermanently }: ArchiveCardProps) {
  return (
    <GlassCard className="flex items-center gap-4 p-3">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <Button variant="secondary" size="sm" onClick={() => onRestore(entry)}>
          Restore
        </Button>
        <Button variant="ghost" size="sm" onClick={() => onDeletePermanently(entry)}>
          Delete
        </Button>
      </div>
    </GlassCard>
